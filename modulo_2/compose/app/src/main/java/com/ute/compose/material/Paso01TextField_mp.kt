...
fun Paso01_TextFieldScreen_mp() {
    Column(
        ...
        DemoBusquedaEvento()
        HorizontalDivider()
        DemoFormularioOrganizador()
    }
}

// ── Demo 1: búsqueda de eventos con limpiar ──────────────────────────────────
@Composable
private fun DemoBusquedaEvento() {
    var busqueda by remember { mutableStateOf("") }

    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        Text("Búsqueda de eventos con ícono y botón limpiar",
        ...
        Text(
            text  = if (busqueda.isBlank()) "Escribe para filtrar eventos"
                    else "Buscando evento: \"$busqueda\"",
...
// ── Demo 2: formulario registro de organizador ───────────────────────────────
@Composable
private fun DemoFormularioOrganizador() {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        Text("Registro de nuevo organizador",
            style = MaterialTheme.typography.labelLarge,
            color = MaterialTheme.colorScheme.primary)

        // Nombre del organizador — validación básica de longitud
        OutlinedTextField(
            value           = nombre,
        ...
        OutlinedTextField(
            ...
            placeholder     = { Text("organizador@empresa.com") },
        ...
        Button(
            ...
            Text(if (formularioValido) "Registrar organizador ✓" else "Completa todos los campos")
        }
    }
}
...
fun Paso01_Preview_mp() {
    MaterialTheme { Paso01_TextFieldScreen_mp() }
}
